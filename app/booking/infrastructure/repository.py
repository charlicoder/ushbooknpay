...
class BookingRepository:
    ...
    async def generate_booking_number(self, for_date: date | None = None) -> str:
        """
        Generate the next unique booking number for the given date.

        Format: B{YY}{MM}{DD}{NNN}
        Example: B260921001 → first booking on 2026-09-21

        The counter (NNN) is the count of bookings that already have a
        booking_number assigned for that calendar day + 1.
        Numbers wrap at 999 — if ever exceeded the counter simply exceeds
        three digits (e.g. 1000) so uniqueness is never broken.
        """
        target_date = for_date or date.today()

        # Count bookings that already have a booking_number for this date.
        # We match on the date-prefix (e.g. "B260921") to be safe, so even
        # numbers generated by other means that share the prefix are counted.
        date_prefix = "B" + target_date.strftime("%y%m%d")
