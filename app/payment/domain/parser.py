...
Extracts and normalizes key transaction identifiers, status, and payment method
from a raw gateway response. All other raw data is preserved in payment_data.
"""
...
def parse_gateway_response(payload: dict[str, Any]) -> dict[str, Any]:
    """
    Parse a full gateway response dictionary into a standardized payment dictionary.
    ...
    Returns a dict that maps directly to Payment model fields.
    All raw/extra gateway data is preserved in the 'payment_data' key.
    """
    ...
    is_paid_flag = (
        payload.get("isPaid") is True
        or payload.get("IsSuccess") is True
        or payload.get("is_paid") is True
    )
    ...
    if is_paid_flag or invoice_status == "paid" or "succ" in txn_status or invoice_status in ("success", "captured"):
    ...
    invoice_value = _safe_decimal(
        data.get("InvoiceValue") or first_txn.get("DueValue") or raw_amount,
        amount,
    )

    # ── 3. Gateway & Transaction Identifiers ──────────────────────────────
    invoice_id = str(
        data.get("InvoiceId") or payload.get("invoiceId") or payload.get("InvoiceId") or ""
    ).strip() or None

    transaction_id = str(
        first_txn.get("TransactionId") or payload.get("transaction_id") or ""
    ).strip() or None
    ...
    # Raw gateway status string (verbatim)
    transaction_status = str(first_txn.get("TransactionStatus") or "").strip() or None

    # ── 4. Payment Method & Gateway ───────────────────────────────────────
    ...
    if gateway_name:
        gn_up = gateway_name.upper()
        if "KNET" in gn_up or "K-NET" in gn_up:
    ...
    # ── 5. Customer Data (stored in customer_data snapshot) ───────────────
    ...
    customer_data_from_gateway = {
        "name": customer_name or None,
        "mobile": customer_mobile or None,
        "phone_number": customer_mobile or None,
        "email": customer_email or None,
    }
    # Remove None values
    customer_data_from_gateway = {k: v for k, v in customer_data_from_gateway.items() if v}

    # ── 6. URLs & Timestamps ──────────────────────────────────────────────
    payment_url = str(payload.get("paymentUrl") or data.get("PaymentURL") or "").strip() or None
    transaction_date_raw = (
        str(first_txn.get("TransactionDate") or payload.get("transaction_date") or "").strip() or None
    )
    paid_at_raw = transaction_date_raw or str(data.get("CreatedDate") or "").strip() or None
    paid_at = _safe_datetime(paid_at_raw)

    # ── 7. Country / Geo ──────────────────────────────────────────────────
    country = str(first_txn.get("Country") or "").strip() or None

    # ── 8. Build payment_data blob (full audit data) ───────────────────────
    # Consolidate all provider-specific identifiers and raw card/ip data into payment_data
    ...
    invoice_reference = str(data.get("InvoiceReference") or "").strip() or None
    customer_reference = str(data.get("CustomerReference") or "").strip() or None
    ...
    payment_data = {
        ...
        "provider_reference": invoice_reference or invoice_id,
        "provider_transaction_id": transaction_id,
        "invoice_reference": invoice_reference,
        "customer_reference": customer_reference,
        "authorization_id": authorization_id,
        "payment_id_gateway": payment_id,
    ...
    return {
        ...
        "country": country,
        "customer_data": customer_data_from_gateway or None,
        "paid_at": paid_at,
