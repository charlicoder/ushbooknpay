...
down_revision = '0014_shop_payment_classification'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not is_offline:
        ...
        def index_exists(t: str, idx_name: str) -> bool:
            if not table_exists(t):
                return False
            return idx_name in {idx["name"] for idx in inspector.get_indexes(t)}

        def column_exists(t: str, col_name: str) -> bool:
            if not table_exists(t):
                return False
            return col_name in {col["name"] for col in inspector.get_columns(t)}

        def uq_exists(t: str, uq_name: str) -> bool:
            if not table_exists(t):
                return False
            return uq_name in {uq["name"] for uq in inspector.get_unique_constraints(t)}
    else:
        def table_exists(t: str) -> bool:
            return False
    ...
    # Gift voucher carts
    if not table_exists('gift_voucher_carts'):
        op.create_table(
            'gift_voucher_carts',
    ...
    if not index_exists('gift_voucher_carts', 'ix_gift_carts_customer_status'):
        op.create_index('ix_gift_carts_customer_status', 'gift_voucher_carts', ['customer_id', 'status'])

    # Gift voucher cart items
    if not table_exists('gift_voucher_cart_items'):
        op.create_table(
    ...
    if not index_exists('gift_voucher_cart_items', 'ix_gift_cart_items_cart'):
        op.create_index('ix_gift_cart_items_cart', 'gift_voucher_cart_items', ['cart_id'])

    # Gift voucher purchases
    if not table_exists('gift_voucher_purchases'):
        op.create_table(
    ...
    if not index_exists('gift_voucher_purchases', 'ix_gift_purchases_sender_status'):
        op.create_index('ix_gift_purchases_sender_status', 'gift_voucher_purchases', ['sender_id', 'status'])

    # Gift voucher purchase items
    if not table_exists('gift_voucher_purchase_items'):
        op.create_table(
    ...
    if not table_exists('gift_voucher_recipients'):
        op.create_table(
    ...
    if not table_exists('gift_voucher_deliveries'):
        op.create_table(
    ...
    if not table_exists('gift_voucher_verifications'):
        op.create_table(
    ...
    if not table_exists('gift_voucher_redemptions'):
        op.create_table(
    ...
    if not column_exists('gift_vouchers', 'gift_type'):
        op.add_column('gift_vouchers', sa.Column('gift_type', sa.String(length=20), server_default='SERVICE', nullable=True))
    if not index_exists('gift_vouchers', 'ix_gift_vouchers_gift_type'):
